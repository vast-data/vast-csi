...
void NlmServer::test(RpcRequest *request, nlm4_testargs *arg, nlm4_testres *res)
{
    ...
    if (eres != EStoreRes::OK) {
        PT_DEBUG(DATA, "nlm4_test on handle=%lx failed res=%d", handle, eres);
        ...
        holder->l_len = existing_lock.end == std::numeric_limits<uint64_t>::max() ? 0 : existing_lock.end - existing_lock.start;
...
void NlmServer::lock(RpcRequest *request, nlm4_lockargs *arg, nlm4_res *res)
{
    ...
    nlm4_lock_to_handle(&arg->alock, &handle);
    nlm4_lock_to_lock_info(arg->exclusive, &arg->alock, &lock);
    EStoreRes eres = _estore->lock(nullptr, nullptr, handle, &lock);
    ...
    if (eres != EStoreRes::OK) {
        PT_DEBUG(DATA, "nlm4_lock on handle=%lx failed res=%d", handle, eres);
...
void NlmServer::unlock(RpcRequest *request, nlm4_unlockargs *arg, nlm4_res *res)
{
    ...
    if (eres != EStoreRes::OK) {
        PT_DEBUG(DATA, "nlm4_unlock on handle=%lx failed res=%d", handle, eres);
        res->stat.stat = NLM4_DENIED;
    }
