...
from lib.k8s.ops_recording import CreationRecorder
from lib.constants import CSI_NAMESPACE
from lib.logging import logger


class _ManifestDumper(yaml.SafeDumper):
    """Quote numeric-looking strings so kubectl does not parse passwords as ints."""


def _represent_str(dumper, data):
    if data and (data[0].isdigit() or data.lower() in {"true", "false", "null", "yes", "no", "on", "off"}):
...
class KubernetesResource:
    ...
    def __init__(self, k8s: "K8S", resource_type: Optional[str] = None):
        self.k8s = k8s
    ...
    def apply(self, objects):
        ...
        with fpath.open("w") as f:
            yaml.dump_all(unbunchify(objects), f, Dumper=_ManifestDumper)
    ...
    def wait(
        ...
        def _wait_present():
            ...
            try:
                ...
                # VolumeSnapshots stay status-less until snapshot-controller reconciles.
                if self.resource_type in ("volumesnapshot", "volumesnapshotcontent"):
                    raise self._wait_failed(
                        num_failed=num_total, num_total=num_total, condition=condition,
                        name=name, namespace=namespace, labels=labels,
                        res_list=res_list, error_msg=error_msg,
                    )
            ...
            if hasattr(statuses[0], "bucketReady"):
            ...
            elif self.resource_type in ("volumesnapshot", "volumesnapshotcontent"):
                num_failed = len([r for r in res_list if not r["status"].get("readyToUse")])
            else:
                ...
                num_failed = len([
                    ...
                    if (
                        ...
                        or (
                            self.resource_type == "pod"
                            and not all(
                                c.get("ready")
                                for c in (r["status"].get("containerStatuses") or [{}])
                            )
                        )
                    )
                ])
...
class K8S:
    ...
    def resource(self, resource_type: str) -> KubernetesResource:
        """Return a manager for any kubectl resource type (Job, DataVolume, CRDs, ...)."""
        return KubernetesResource(self, resource_type=resource_type)

    def apply(self, objects) -> None:
        """Apply one or more manifests (any kind) via kubectl/oc."""
        if not isinstance(objects, (list, tuple)):
            objects = [objects]
        fpath = self._next_object_yaml_path("manifest")
        with fpath.open("w") as f:
    ...
    @cached_property
    def namespaces(self):
        from lib.k8s.workloads import Namespace
        return Namespace(self)

    @cached_property
    def pods(self):
        from lib.k8s.workloads import Pod
        return Pod(self)
    ...
    @cached_property
    def sts(self):
        from lib.k8s.workloads import StatefulSet
        return StatefulSet(self)

    @cached_property
    def pvcs(self):
        from lib.k8s.storage import PersistentVolumeClaim
        return PersistentVolumeClaim(self)

    @cached_property
    def pvs(self):
        from lib.k8s.storage import PersistentVolume
        return PersistentVolume(self)

    @cached_property
    def storageclasses(self):
        from lib.k8s.storage import StorageClass
        return StorageClass(self)

    @cached_property
    def volumesnapshots(self):
        from lib.k8s.storage import VolumeSnapshot
        return VolumeSnapshot(self)

    @cached_property
    def volumesnapshotcontents(self):
        from lib.k8s.storage import VolumeSnapshotContent
