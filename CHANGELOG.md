...
## Version 2.6.5-hf8
* Added opt-in NFS client-services node sidecar for node OSes that do not ship rpcbind/rpc.statd (e.g. VKS, NKP). Enable via `node.nfsServices.enabled`; configurable daemons via `node.nfsServices.services` (default: statd, rpcbind). Fixes NFSv3 mounts failing with "rpc.statd is not running"
* Updated base container image to include `rpcbind` and `ktls-utils` packages required by the NFS client-services sidecar

## Version 2.6.5-hf7
* Updated Python 3.12 RPMs in the base container image build to pick up latest UBI security fixes (VCSI-575)
* Updated Python dependencies for security fixes, including `cryptography` >= 48.0.1
