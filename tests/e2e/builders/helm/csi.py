...
from e2e.constants import CSI_NAMESPACE, NFS_MOUNT_OPTIONS, PASSWORD, SNAPSHOT_CLASS, USERNAME, nfs_storage_class
...
class VastCsiHelmValuesBuilder(HelmValuesBuilder):
    ...
    def for_fleet(cls, system, *, namespace: str = CSI_NAMESPACE) -> Self:
        return (
            ...
            .with_deletion_resources(system.vippool_name, system.view_policy_name)
            .with_attach_required(True)
            .with_nfs_services(True)
            .with_nfs_mount_options()
            .with_storage_classes(system, namespace=namespace)
            .with_snapshot_classes(system, namespace=namespace)
        )
    ...
    def with_storage_classes(self, system, *, namespace: str = CSI_NAMESPACE) -> Self:
        storage_classes = self._values.setdefault("storageClasses", {})
        storage_classes[nfs_storage_class(0)] = self._nfs_sc_params(system, namespace)
        storage_classes[nfs_storage_class(1)] = self._nfs_sc_params(
            system, namespace, extra=True,
        )
        return self

    @staticmethod
    def _nfs_sc_params(system, namespace: str, *, extra: bool = False) -> dict[str, Any]:
        params = {
            "storagePath": system.root_export if not extra else f"{system.root_export.rstrip('/')}-1",
            "viewPolicy": system.view_policy_name,
            "mountOptions": list(NFS_MOUNT_OPTIONS),
            **vippool_fields(system.vippool_name),
    ...
    def with_snapshot_classes(self, system, *, namespace: str = CSI_NAMESPACE) -> Self:
