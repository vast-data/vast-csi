...
func RegisterFlags(c *cobra.Command, cfg *config.Config) {
	c.PersistentFlags().BoolVar(&cfg.ApplyExistingPVCs, "apply-existing-pvcs", false,
	...
	c.PersistentFlags().StringVar(&cfg.PVCNameFormat, "pvc-name-format", common.DefaultPVCNameFormat,
  ...
  {sc_name_pref:<N>}    - first N characters of StorageClass name`)
	...
	c.PersistentFlags().StringVar(&cfg.PVNameFormat, "pv-name-format", common.DefaultPVNameFormat,
  ...
  {sc_name_pref:<N>}    - first N characters of StorageClass name`)
	...
	c.PersistentFlags().StringVar(&cfg.VolumeReplicationNameFormat, "volume-replication-name-format", "{vr_name}-repl-{endpoint}",
  ...
  {sc_name_pref:<N>}    - first N characters of StorageClass name`)
	...
	c.PersistentFlags().StringVar(&cfg.VolumeGroupReplicationNameFormat, "volume-group-replication-name-format", "{vgr_name}-repl-{endpoint}",
...
// configure registers replication controllers on the shared manager.
