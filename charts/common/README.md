...
The library is versioned independently of the public charts and is not published as its
own Helm chart. Dependents pin it with `repository: "file://../common"` so
`helm dependency build` / `make chart-deps` packages this directory into each parent
chart. Release CI vendors that tarball into the published parent `.tgz`; customers
installing from the Helm repository do not fetch `vast-common` separately. Release CI
does not stamp this chart's version.
