...
void TimerQueues::poll()
{
    ...
    LOOP(SleepInterval::SLEEP_INTERVAL_COUNT, i) {
        DList::Anchor* anchor = &scheduler->_timer_queues._queues[i];
        DList queue;
        queue.init(anchor, &scheduler->_fiber_queues);
        while (!queue.is_empty()) {
            Fiber *fiber = (Fiber*) scheduler->_fiber_pool.index_to_address(queue.get_first());
            uint64_t fiber_wakeup = fiber->_switch_time + MICRO_TO_NANO(interval_to_micro[i]);
