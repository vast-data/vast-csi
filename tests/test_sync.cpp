...
#include "plasma/sync/rwlock.hpp"
...
TEST(TestSync, test_rwlock_barrier)
{
    RWlock lock;
    lock.init();
...
TEST(TestSync, test_rwlock_simple)
{
    RWlock lock;
    lock.init();
...
TEST(TestSync, test_sem_nonblocking)
{
    Sem sem;
    sem.init(2);
...
TEST(TestSync, test_sem_blocking)
{
    Sem sem;
    sem.init(0);
